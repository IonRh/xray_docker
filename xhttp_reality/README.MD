...
## 持久化配置（推荐）
使用宿主机目录保存 UUID/私钥/路径等信息，镜像更新后连接信息保持不变：

```
docker pull wulabing/xray_docker_xhttp_reality:latest
EXTERNAL_PORT=443
docker run -d --name xray_xhttp_reality --restart=always --log-opt max-size=100m --log-opt max-file=3 -p $EXTERNAL_PORT:443 -e EXTERNAL_PORT=$EXTERNAL_PORT -v "./wulabing_xray_config:/data" wulabing/xray_docker_xhttp_reality:latest
```

如果你之前保存过旧版的 config_info.txt，可放到 `./wulabing_xray_config/config_info.txt` 或 `./wulabing_xray_config/xhttp_reality_config_info.txt`，启动时会自动兼容读取。
