...
## 持久化配置（推荐）
使用宿主机目录保存 UUID/私钥/订阅信息，镜像更新后连接信息保持不变：
...
如果你之前保存过旧版的 config_info.txt，可放到 `./wulabing_xray_config/config_info.txt` 或 `./wulabing_xray_config/reality_config_info.txt`，启动时会自动兼容读取。
